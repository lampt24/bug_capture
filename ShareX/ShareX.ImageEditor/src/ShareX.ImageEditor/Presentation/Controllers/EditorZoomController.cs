...
public class EditorZoomController
{
    ...
    public bool ZoomToWidth()
    {
        if (_view.DataContext is not MainViewModel vm) return false;
        ...
        double contentWidth = previewFrame.Bounds.Width;
        if (contentWidth <= 0) contentWidth = vm.ImageWidth;

        if (contentWidth <= 0) return false;

        // Use a larger margin for width to account for potential vertical scrollbar
        const double widthMargin = 20;
        double availableWidth = Math.Max(1, scrollViewer.Viewport.Width - widthMargin);
        
        double fitZoom = availableWidth / contentWidth;
        
        // Don't zoom in automatically beyond 100% for the initial fit
